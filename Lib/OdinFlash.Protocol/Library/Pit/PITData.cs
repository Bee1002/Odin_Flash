...
namespace OdinFlash.Protocol.Pit
{
	...
	public class PITData
	{
		...
		{
			string text = "";
			if (sData == null || sData.Length < (int)TFileIdentifier.kHeaderDataSize)
			{
				ClearEntries();
				return false;
			}
			...
			long maxEntries = (sData.Length - (int)TFileIdentifier.kHeaderDataSize) / (int)TPitEntry.kDataSize;
			if (this.entryCount <= 0 || this.entryCount > maxEntries)
			{
				ClearEntries();
				return false;
			}
		...
		{
			this.entryCount = 0L;
			this.xPIT_Entry = new TPIT_Entry[0];
			this.PIT_EntryOrdered = new TPIT_Entry[0];
		}
